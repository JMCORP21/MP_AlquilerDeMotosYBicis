    ...
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
				if tieneunabicimp == false then

			--		helptext(_U('press_e'))

			local playerPed = PlayerPedId()
			local playerX, playerY, playerZ = table.unpack(GetEntityCoords(playerPed))
			ESX.ShowFloatingHelpNotification(_U('press_e'), vector3(playerX, playerY, playerZ + 1))
					
					if IsControlJustPressed(0, Keys['E']) and IsPedOnFoot(ped) then
						OpenBikesMenu()
					end 
				elseif tieneunabicimp == true then

			--		helptext(_U('storebike'))
			local playerPed = PlayerPedId()
			local playerX, playerY, playerZ = table.unpack(GetEntityCoords(playerPed))
			ESX.ShowFloatingHelpNotification(_U('storebike'), vector3(playerX, playerY, playerZ+1))
...
function OpenBikesMenu()
	...
	if data.current.value == 'bike5' then
		if Config.EnablePrice then
			TriggerServerEvent("esx:bike:lowmoney", Config.Preciomoto) 
			TriggerEvent("chatMessage", _U('bikes'), {255,0,255}, _U('bike_pay', Config.Preciomoto))
		end
		
		if Config.EnableEffects then
			spawn_effect("vindicator")
		else
			TriggerEvent('esx:spawnVehicle', "vindicator")
		end
	end
